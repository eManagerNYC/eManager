...
class elFinder {
	...
	public function __construct($opts) {
        ...
        $folderfoall = get_option('filemanager_folder_for_all', true);
        if($folderfoall=='yes')
        {
            $this->fileaccess = (int)get_option('filemanager_folder_for_all_readwrite', true);
    		
        }
        else
        {
            $user_id = get_current_user_id();
            $this->fileaccess = (int)get_user_meta($user_id, 'file_manager_readwrite', true);
        }
        
        if($this->fileaccess==1)
        {
            //read+write
            $this->commands = array(
	...
	protected function file($args) {
		...
		if ($download) {
			...
			$disp  = $file['mime'] == 'application/x-shockwave-flash'
            //$disp  = preg_match('/^(image|text)/i', $file['mime']) || $file['mime'] == 'application/x-shockwave-flash' 
					? 'inline' 
					: 'attachment';
			$mime = $file['mime'];
		}
		//$filenameEncoded = encodethis($file['name']);
