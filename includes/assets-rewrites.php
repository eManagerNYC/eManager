 ...
 */
add_action( 'after_setup_theme', 'eman_assets_rewrites' );
function eman_assets_rewrites()
{
	/**
	...
	function eman_clean_urls( $content )
	{
	...
	{
		add_action('generate_rewrite_rules', 'eman_add_rewrites');
		...
		if ( ! is_admin() )
		{
			...
			foreach ( $tags as $tag ) {
				add_filter($tag, 'eman_clean_urls');
			}
		}
	}
}